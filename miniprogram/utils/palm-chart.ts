...
/** 情感线为凹弧：两端偏高（食指侧最高、小指侧微翘），中段向智慧线方向下凹（真实掌形；2026-09-08 重画，原为凸拱被用户指出） */
const PATH_HEART =
  'm196 259C218 265 256 279 284 288C306 292 342 292 366 280';
const PATH_HEAD =
...
/** 辅线基线：缘分线（小指下方短横）、灵感线（自中部斜向上）。控制点取自掌内实测安全区：小指侧指缝空腔（x≈375-400）下探到 y≈270+，须避开；小指柱 x≈405-450 自 y≈210 起全在掌内，可放短横。灵感线上端须停在 x≤366 且 y≥294（凹弧情感线右端下方的净距区） */
function auxBase(qid: 'bond' | 'insight'): Pt[] {
  if (qid === 'bond') return synthStroke({ x: 405, y: 272 }, { x: 422, y: 248 }, { x: 440, y: 256 });
  return synthStroke({ x: 316, y: 340 }, { x: 338, y: 316 }, { x: 358, y: 294 });
}
