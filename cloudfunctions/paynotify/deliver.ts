/**
 * 幂等发货 —— paynotify 主入口。
 * ⚠ 与 cloudfunctions/pay/deliver.ts 保持同步（云函数包互相独立无法共享代码，
 *   config 由 tests/pay-config-consistency.test.ts 校验；逻辑改动两边同改）。
 *
 * 幂等策略：
 * 1. pay_deliver_log 以微信支付单号 wxOrderId 为 _id（重复 _id 写入失败 = 已发货）；
 *    同时按 outTradeNo 反查，防止「推送按 MchOrderNo、查单兜底按 outTradeNo」双路重复入账。
 * 2. 配额入账失败时回滚删除锁记录并返回 error，让平台重推（≤15 次）可完整重试。
