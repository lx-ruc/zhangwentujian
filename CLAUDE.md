...
Product: WeChat Mini Program **十二人格签** — a "fun personality test" (趣味测试) built as a closed 12-archetype sign-collection, driven by a 6-question 掌纹 quiz. NO camera anywhere. Deliberately positioned as entertainment, NOT fortune-telling. (Renamed from 掌纹测运 on 2026-09-07 after a second rejection; camera flow deleted entirely on 2026-09-08 after the fourth rejection screenshotted the capture page. Console name 「AI掌纹分析」 kept by user decision; see Compliance below.)
...
- **Quiz engine**: LOCAL, no runtime AI. `miniprogram/data/quiz-questions.ts` (question bank: 惯用手 + 3 main lines 情感线/思维线/活力线 + 2 aux lines 缘分线/灵感线) + `miniprogram/utils/quiz.ts` (deterministic FNV-1a seed over answers → score bands → `classify.ts` maps to one of 12 types); `utils/draw.ts` now keeps only `demoReport()` (report-page fallback). The Zhipu GLM-4.6V-Flash vision pipeline in `cloudfunctions/analyze` is DORMANT (kept as dead code, guarded by `tests/e2e-real-model.test.ts`); if ever revived, API key lives ONLY in cloud function env vars
...
Data flow (2026-09-08 pivot, B-line implemented): `index` (text CTA 开始测试) → `quiz` (6 questions; each option card renders a palm-outline chart via canvas `utils/palm-chart.ts` — the asked line cinnabar, context lines faint ink, unselected options ink; left hand mirrored) → local `quizOutcome` deterministic (~0.9s ritual) → cloud fn `analyze` action `draw`: quota check → `validateReport` shape+banned-term check → typeId whitelist → persist text-only report to `analyses` with `modelVersion:'local-draw-1'` → consume quota → `report` (renders ALL body copy from `data/report-content.ts` via `REPORT_CONTENT[type.id]`).

Key decisions that span multiple files:

- **No camera anywhere**: `capture`/`analyzing` pages are deleted (2026-09-08); the app never touches `wx.chooseMedia` or camera permission, no photo exists at any point in the flow. DB stores text reports only. Never reintroduce photo capture or upload.
...
- **Sign collection (core viral asset)**: 12 archetypes in `miniprogram/data/palm-types.ts` (No.01-12, rarity, tagline, compat). Type is determined by quiz option indices: each main-line option carries a disjoint score band (深长贯通 90-92 > 明快上扬 73-77 > 平缓舒展 52-55 > 浅淡纤细 45-48), so aux lines + 惯用手 only jitter within band and never change the type; `utils/classify.ts` maps scores back to type and is inverse-consistent (test-locked, 864 combos reach all 12). Full per-type copy lives in `miniprogram/data/report-content.ts` (`REPORT_CONTENT`, closed set). There is NO runtime AI anywhere in the live path.
...
WeChat bans 算命/占卜/看相 content as 封建迷信. Four review rejections so far (2026-08-24: 算命内容 + 深度合成/个人主体; 2026-09: CTA 「拍摄手掌·开始测试」 ordered removed → surface-clean + rename kept the photo flow; 2026-09-08: active user 举报 算卦算运势 + rejection screenshotted the CAMERA page). Response (2026-09-08 decisions, user-approved): delete `capture`/`analyzing` entirely, replace with a 6-question 掌纹 quiz (user self-reports line shapes — no system "reading" of any image), keep console name 「AI掌纹分析」. The flow shape 「拍手照→性格报告」 is gone; there is nothing for a reviewer to screenshot as photo-based fortune-telling.

1. **Banned vocabulary** — single source of truth is `tests/copy-ban.test.ts`: 算命 占卜 手相 面相 大师 运势 运气 好运 转运 旺 命运 吉 凶 灾 祸 求签 签文 解签 测运 + palm set 手掌 掌心 巴掌 手纹 + **single char 掌 via regex `掌(?!纹)`** — the exact word 「掌纹」 is the ONLY allowed palm term (the product IS a 掌纹 quiz, per user decision 2026-09-08), so 鼓掌/掌握 etc. remain banned but 掌纹问答/掌纹形态 pass. Also banned: `/AI生成|AI解读|AI分析|AI读取/` and visible English `/palm/i`. `tests/classify.test.ts` keeps a STRICTER list for `REPORT_CONTENT` (all palm words incl. 掌纹 — report copy never contains them), and `cloudfunctions/analyze/validate.ts` stays strictest of all (server-side filter; quiz copy is never sent to the server). The scanner sweeps every user-visible string in `miniprogram/**` + both design HTMLs; identifiers/comments/`{{bindings}}`/import paths are out of scope. UI copy must never claim AI 生成/读取/分析. 「签」 is fine as a noun (人格签); banned sign-verbs are 求签/解签 — use 抽取/解锁.
...
5. **B-line contingency — IMPLEMENTED 2026-09-08**: `capture`/`analyzing` deleted, quiz flow live. If a FUTURE rejection still targets 掌纹 quiz content itself (shape: 掌纹问答→性格报告), the remaining fallback is dropping the 掌纹 frame entirely — abstract the 6 questions into neutral shape-preference picks (无「掌纹」字样) keeping the same engine. Do not pre-emptively implement; wait for the trigger.
